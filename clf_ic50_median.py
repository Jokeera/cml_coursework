...
TARGET_IC50_COLUMN = 'IC50_nM' # ИЗМЕНЕНО
USE_FEATURE_SELECTION = True
N_TOP_FEATURES_TO_SELECT = 99
TASK_NAME = "clf_ic50_median"
TUNE_BASE_MODELS = True

DATA_FILE = "data/eda_gen/data_final.csv"
FEATURES_FILE = "data/eda_gen/features/clf_IC50_gt_median.txt" # ИЗМЕНЕНО
...
def main():
    ...
    # Бинаризация таргета
    median_val = df_full[TARGET_IC50_COLUMN].median()
    y = (df_full[TARGET_IC50_COLUMN] > median_val).astype(int)
    y.name = "target"
    logger.info(f"🎯 Цель: {TARGET_IC50_COLUMN} > {median_val:.2f}, баланс классов:\n{y.value_counts(normalize=True).rename('proportion')}")
    ...
    # ИСПРАВЛЕНИЕ 1: Удалена лишняя переменная X_df. Теперь везде будет использоваться только X.
    # X_df = X.copy() # <--- ЭТА СТРОКА УДАЛЕНА
    ...
    plt.grid(True)
    plt.tight_layout()
    roc_curve_path = os.path.join(TASK_PLOTS_DIR, f"{TASK_NAME}_feature_selection_curve.png")
    ...
    # Подсчёт class_weights вручную для IC50
    ...
    plt.legend()
    roc_path = os.path.join(TASK_PLOTS_DIR, f"{TASK_NAME}_OOF_Best_{best_model_name}_roc_curve.png")
    ...
    if isinstance(model_for_shap, StackingClassifier):
        ...
        try:
            # Для SHAP explainer нужно передать трансформированные данные
            ...
            # Логика из логов: было отобрано 97 признаков
            final_features = feature_importance['feature'].head(97).tolist() 
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(TASK_PLOTS_DIR, f"{TASK_NAME}_overfitting_check_auc.png"))
    plt.close()
    ...
    plt.legend()
    roc_path = os.path.join(TASK_PLOTS_DIR, f"{TASK_NAME}_holdout_ROC.png")
    plt.savefig(roc_path)
    plt.close()
