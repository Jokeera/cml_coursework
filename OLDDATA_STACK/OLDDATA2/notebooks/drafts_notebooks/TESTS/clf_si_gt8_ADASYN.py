...
def preprocess_data(df):
    ...
    features = df.drop(columns=[col for col in forbidden_cols if col in df.columns], errors='ignore')
...
def train_and_evaluate(X, y):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        X_train_prep = pipeline.fit_transform(X_train_adasyn)
        X_val_prep = pipeline.transform(X_val)

        model.fit(X_train_prep, y_train_adasyn, eval_set=(X_val_prep, y_val))
