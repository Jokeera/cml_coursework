...
TARGET_CC50_COLUMN = 'CC50' # ЗАДАЧА: CC50_nM > медианы CC50_nM
...
def main():
    ...
    if TARGET_CC50_COLUMN not in df.columns:
        logger.error(f"Целевая колонка '{TARGET_CC50_COLUMN}' не найдена. Доступные колонки: {df.columns.tolist()}")
        return

    # === Создание целевой переменной ===
    median_val = df[TARGET_CC50_COLUMN].median()
    y = (df[TARGET_CC50_COLUMN] > median_val).astype(int)
    y.name = "target"
    median_display = (
        f"{median_val:.2e}" if abs(median_val) > 1e4 or (abs(median_val) < 1e-2 and median_val != 0)
        else f"{median_val:.2f}"
    )
    logger.info(f"Задача: '{TARGET_CC50_COLUMN} > {median_display}'. Баланс классов:\n{y.value_counts(normalize=True).rename('proportion')}")

    # === Удаление целевой переменной и утечек ===
    df = df.drop(columns=[TARGET_CC50_COLUMN], errors='ignore')
    forbidden_cols = [
        'IC50_mM', 'CC50_mM', 'SI_corrected',
        'log_IC50', 'log_CC50', 'log_SI', 'log1p_IC50_nM', 'log1p_SI_corrected',
        'SI_original', 'SI_diff', 'SI_diff_check', 'SI_check',
        'ratio_IC50_CC50','SI', 'IC50'
    ]
