...
TARGET_IC50_COLUMN = 'IC50' # ЗАДАЧА: IC50 > медианы IC50
...
def main():
    ...
    if TARGET_IC50_COLUMN not in df.columns:
        logger.error(f"Целевая колонка '{TARGET_IC50_COLUMN}' не найдена. Доступные колонки: {df.columns.tolist()}")
        return
    ...
    y.name = "target"
    median_display = (
        f"{median_val:.2e}" if abs(median_val) > 1e4 or (abs(median_val) < 1e-2 and median_val != 0)
    ...
    # === Удаление целевой переменной и утечек ===
    df = df.drop(columns=[TARGET_IC50_COLUMN], errors='ignore')
    forbidden_cols = [
        'IC50_mM', 'CC50', 'CC50_mM', 'SI', 'SI_corrected',
        'log_IC50', 'log_CC50', 'log_SI',
        'log1p_IC50_nM', 'log1p_SI_corrected',
    ...
    if TUNE_BASE_MODELS:
        ...
        for name, model, param_grid in base_learners_config:
            ...
            # ВАЖНО: имя шага 'classifier' должно совпадать с тем, как мы обращаемся к параметрам в param_grid_with_prefix
        ...
        for name, model_class_constructor, _ in base_learners_config: # model_class_constructor это XGBClassifier или CatBoostClassifier
             # Используем имя 'classifier' для шага, как и при тюнинге
    ...
    # Имена для StackingClassifier должны быть уникальны, если передаем список кортежей
    # tuned_processed_base_learners уже содержит ('catboost', Pipeline(...)), ('xgboost', Pipeline(...))
    stacking_clf = StackingClassifier(
        estimators=tuned_processed_base_learners, # Это уже список вида [('catboost', пайплайн1), ('xgboost', пайплайн2)]
    ...
    for model_name_key, score in best_single_models_oof_scores.items(): # Используем ключи из словаря
    ...
    try:
        ...
        X_df_shap = pd.DataFrame(X_processed_shap_np, columns=feature_names_for_model) # Имена отобранных или всех признаков
        ...
        if hasattr(final_trained_model, 'estimators_') and final_trained_model.estimators_:
            for i, base_pipeline_estimator in enumerate(final_trained_model.estimators_):
                # base_pipeline_estimator это Pipeline([('preprocessor', ...), ('classifier', ...)])
                base_est_original_name = base_learners_config[i][0] # 'catboost' или 'xgboost'
                
                try:
                    # Имя шага модели в пайплайне - 'classifier'
    ...
    artifacts = {'model': final_trained_model, 'features': feature_names_for_model, 
                 'optimal_threshold_oof': optimal_thr_stacking, 'preprocessor': shap_preprocessor} # Сохраняем препроцессор, обученный на всем X
