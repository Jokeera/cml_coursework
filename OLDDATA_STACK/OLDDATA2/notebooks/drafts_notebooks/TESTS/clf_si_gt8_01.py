...
def preprocess_data(df):
    ...
    leakage_cols = [
        "IC50", "CC50", "IC50_mM", "CC50_mM", "SI_corrected",
        "log_IC50", "log_CC50", "log_SI", "log1p_IC50_nM", "log1p_SI_corrected",
        "SI_original", "SI_diff", "SI_diff_check", "SI_check", "ratio_IC50_CC50", "SI"
    ]
...
def train_and_evaluate(X, y):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        sampler = SMOTE(random_state=RANDOM_STATE) if USE_SMOTE else ADASYN(random_state=RANDOM_STATE)
        logger.info("SMOTE применяется внутри фолда." if USE_SMOTE else "ADASYN применяется внутри фолда.")

        X_train_res, y_train_res = sampler.fit_resample(X_train_prep, y_train)

        model.fit(X_train_res, y_train_res, eval_set=(X_val_prep, y_val))
