...
FINAL_DATA_REGRESS = os.path.join(DATA_DIR, "data_final_reg.csv")
X_SCALED_REG_PATH = os.path.join(DATA_DIR, "X_scaled_reg.csv")
SCALER_REG_PATH = os.path.join(DATA_DIR, "scaler_reg.pkl")
...
if __name__ == '__main__':
    ...
    if loaded_model:
        logger.info(f"Загруженная модель: value={loaded_model.value}, prediction={loaded_model.predict([1,2])}")

    loaded_features = load_model_artifact(test_task_name, test_model_type, 'features')
    if loaded_features:
        logger.info(f"Список признаков: {loaded_features}")

    loaded_threshold = load_model_artifact(test_task_name, test_model_type, 'threshold')
    if loaded_threshold is not None:
        logger.info(f"Порог: {loaded_threshold:.4f}")

    loaded_scaler = load_model_artifact(test_task_name, test_model_type, 'scaler')
    if loaded_scaler:
        logger.info(f"Scaler (тип: {type(loaded_scaler).__name__})")

    logger.info(f"Папки: {MODELS_DIR}, {PLOTS_DIR}, {DATA_DIR}")
