...
def main():
    ...
    forbidden_cols = [
        'IC50_mM', 'CC50_mM', 'SI_corrected', 'log_IC50', 'log_CC50', 'log_SI', 'log1p_IC50_nM',
        'log1p_SI_corrected', 'SI_original', 'SI_diff', 'SI_diff_check', 'SI_check', 'ratio_IC50_CC50', 'SI', 'IC50'
    ]
    ...
    if TUNE_BASE_MODELS:
        ...
        for name, model, param_grid in base_learners_config:
            ...
            logger.info(f"OOF-оценка модели {name}...")
            ...
            logger.info(f"OOF ROC AUC для {name}: {oof_roc_auc_single:.4f}")
    ...
    logger.info(f"Лучший ROC AUC Stacking: {gs_stacking.best_score_:.4f}")

    logger.info("OOF-предсказания для Stacking...")
    ...
    logger.info(f"Confusion matrix (T={optimal_thr_stacking:.2f}):\n{classification_report(y, y_pred_oof_opt_stacking)}")

    cm = confusion_matrix(y, y_pred_oof_opt_stacking)
    plt.figure(figsize=(6,4)); sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar=False)
    plt.title(f'Confusion Matrix (Stacking OOF, T={optimal_thr_stacking:.2f})\\n{TASK_NAME}')
    ...
    for model_name_key, score in best_single_models_oof_scores.items():
        logger.info(f"Модель {model_name_key}: OOF ROC AUC = {score:.4f}")
    logger.info(f"Stacking Classifier: OOF ROC AUC = {oof_roc_auc_stacking:.4f}")

    final_trained_model = best_stacking_model

    logger.info("SHAP-анализ базовых моделей...")
    try:
        shap_preprocessor = Pipeline(steps=[('imputer', SimpleImputer(strategy='median')), ('scaler', RobustScaler())])
        X_shap = shap_preprocessor.fit_transform(X)
        X_df_shap = pd.DataFrame(X_shap, columns=feature_names_for_model)

        if hasattr(final_trained_model, 'estimators_'):
            for i, pipe in enumerate(final_trained_model.estimators_):
                model_name = base_learners_config[i][0]
                try:
                    model = pipe.named_steps['classifier']
                    if isinstance(model, (CatBoostClassifier, XGBClassifier)):
                        explainer = shap.TreeExplainer(model)
                        shap_values = explainer.shap_values(X_df_shap)
                        shap_vals = shap_values[1] if isinstance(shap_values, list) else shap_values
                        plot_shap_summary_custom(shap_vals, X_df_shap, "bar", f"_{model_name}")
                        plot_shap_summary_custom(shap_vals, X_df_shap, "beeswarm", f"_{model_name}")
                except Exception as e:
                    logger.warning(f"Ошибка SHAP ({model_name}): {e}")
    except Exception as e:
        logger.error(f"Ошибка SHAP общего блока: {e}")
